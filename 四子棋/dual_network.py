...
DN_INPUT_SHAPE = (7, 6, 2) # 輸入 shape 四子棋棋盤為7x6的2軸陣列
DN_OUTPUT_SIZE = 7 # 動作數（7)每一回合只能在7個位置選一個放棋

# 建構卷積層
def conv(filters):
    return Conv2D(filters, 3, padding='same', use_bias=False,kernel_initializer='he_normal', kernel_regularizer=l2(0.0005))
    #             卷積核數量、尺寸、啟用填補法、是否增加偏值、  權重矩陣的初始值                 、用於kernel權重常規劃kernel特徵萃取
